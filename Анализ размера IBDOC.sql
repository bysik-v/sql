...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE('START_ZIP---IBDOC-------');

  FOR curs_row IN (SELECT tbl.*,type_doc as name
                      FROM BARS_739.IBDOC tbl
                     WHERE tbl.IB_DOC_XML IS NOT NULL --and rownum<1000
                     order by type_doc) --NEED CHANGE TABLE NAME and BLOB NAME!!!!!!!!
   ...
   LOOP
      ...
      v_tmp_blob_in := curs_row.IB_DOC_XML;           --NEED CHANGE BLOB FIELD NAME!!!!!!!!
   ...
   DBMS_OUTPUT.PUT_LINE('END_ZIP----IBDOC---------');
   ...
   DBMS_OUTPUT.PUT_LINE('END--------IBDOC---------');
