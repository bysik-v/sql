...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE('START_ZIP---IMAGE-------');

  FOR curs_row IN (SELECT tbl.*,nvl(dt.name,'не найден') as name
                      FROM BARS_739.IMAGE tbl LEFT JOIN DOC d
                      ON tbl.doc_id=d.id LEFT JOIN dt
                      ON d.doctype_id=dt.id
                     WHERE tbl.IMAGE IS NOT NULL --and rownum<1000
                     order by dt.name nulls first) --NEED CHANGE TABLE NAME and BLOB NAME!!!!!!!!
   ...
   LOOP
      ...
      v_tmp_blob_in := curs_row.IMAGE;           --NEED CHANGE BLOB FIELD NAME!!!!!!!!
   ...
   DBMS_OUTPUT.PUT_LINE('END_ZIP----IMAGE---------');
   ...
   DBMS_OUTPUT.PUT_LINE('END--------IMAGE---------');
